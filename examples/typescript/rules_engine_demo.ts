...
// Shows advanced rules evaluation for conditional workflow activities
...
async function main() {
  ...
  try {
    ...
    const qualityRules = [
      ...
      RuleBuilder.fieldExists(
        "has_reviewer",
        "Article must have a reviewer assigned",
        "reviewer",
      ),
      RuleBuilder.fieldGreaterThan(
        "word_count_sufficient",
        "Article must have at least 500 words",
        "word_count",
        500,
      ),
      RuleBuilder.fieldGreaterThan(
        "quality_score_high",
        "Article must have quality score > 7",
        "quality_score",
        7,
      ),
      RuleBuilder.fieldContains(
        "has_urgent_tag",
        "Article tagged as urgent",
        "tags",
        "urgent",
      ),
    ];

    const publishReadyRule = RuleBuilder.or(
      "publish_ready",
      "Ready to publish",
      [
    ...
    console.log(
      `  • ${publishReadyRule.name}: ${publishReadyRule.description}`,
    );
    console.log();
    ...
    const testScenarios = [
      {
        ...
        data: {
          ...
          content:
            "This is a comprehensive guide covering all aspects of Rust programming...",
          word_count: 1250,
          quality_score: 9,
    ...
    for (const [index, scenario] of testScenarios.entries()) {
      ...
      logSuccess(`Created article resource: ${resource.data.title}`);
      ...
      const reviewResource = reviewResult.data!.executeActivity;
      ...
      qualityRules.forEach((rule) => {
        const result = ClientRuleEngine.evaluateRule(
          rule,
          reviewResource.data,
          reviewResource.metadata,
        );
        logRule(rule, result);
      });

      // Test composite publish-ready rule
      const publishResult = ClientRuleEngine.evaluateRule(
        publishReadyRule,
        reviewResource.data,
        reviewResource.metadata,
      );
      ...
      if (publishResult.passed) {
        if (reviewResource.data.emergency) {
          nextActivity = "emergency_publish";
          nextData.publishReason =
            "Emergency override - bypassing quality check";
          logWarning("Using emergency publish due to override flag");
        } else {
          nextActivity = "quality_check";
          nextData.qualityCheckPassed = true;
          logInfo("Proceeding to quality check - all criteria met");
        }
      } else {
        nextActivity = "request_revision";
      ...
      // Execute the determined activity
      const finalResult = await client.executeActivity({
      ...
      if (finalResult.errors) {
        logError(
          `Failed to execute activity: ${finalResult.errors.map((e) => e.message).join(", ")}`,
        );
        continue;
      }

      const finalResource = finalResult.data!.executeActivity;
      logSuccess(`Article moved to: ${finalResource.state}`);

      // If approved, publish it
      if (finalResource.state === "quality_check") {
        const approveResult = await client.executeActivity({
          resourceId: finalResource.id,
          activityId: "approve_article",
          data: {
            timestamp: new Date().toISOString(),
        ...
        if (!approveResult.errors) {
          const approvedResource = approveResult.data!.executeActivity;

          const publishFinalResult = await client.executeActivity({
            resourceId: approvedResource.id,
            activityId: "publish_article",
            data: {
              timestamp: new Date().toISOString(),
              publishUrl: `https://blog.example.com/${resource.data.title.toLowerCase().replace(/\s+/g, "-")}`,
            },
          });

          if (!publishFinalResult.errors) {
            logSuccess(`Article published: ${resource.data.title}`);
          }
        }
      }
    }
    ...
    console.log("  • Conditional workflow activities based on rule results");
