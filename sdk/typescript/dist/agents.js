...
export class AgentClient {
     ...
     */
    async create(input) {
        const mutation = operations.agents.create([
            "id",
            "name",
            "description",
     ...
     */
    async get(id) {
        const query = operations.agents.get([
            "id",
            "name",
            "description",
            "llmProvider { providerType model baseUrl }",
     ...
     */
    async list(_options) {
        const query = operations.agents.list([
            "id",
            "name",
            "description",
            "llmProvider { providerType model baseUrl }",
    ...
    `;
        const variables = {
            ...
            input: {
                ...
                capabilities: ["chat", "completion"],
                tools: updates.config?.tools?.map((tool) => ({
                    name: tool.name,
     ...
     */
    async delete(id) {
        const mutation = operations.agents.delete();
    ...
    async chat(id, messages) {
        ...
        const mutation = operations.llm.chatCompletion();
