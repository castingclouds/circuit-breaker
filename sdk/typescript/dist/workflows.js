...
export class WorkflowClient {
    ...
    `;
        const variables = {
            ...
            input: {
                ...
                activities: updates.definition?.transitions.map((t, index) => ({
                    id: `activity_${index}`,
                    name: `${t.from} to ${t.to}`,
                    fromStates: [t.from],
                    toState: t.to,
    ...
    `;
        const result = await this.client.query(query, { id: resourceId });
    ...
    `;
        const result = await this.client.mutation(mutation, { id: executionId });
